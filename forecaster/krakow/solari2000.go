...
type Solari2000 struct {
	BasePath string
}

func NewSolari2000(basePath string) Solari2000 {
	return Solari2000{
		BasePath: basePath,
	}
}

func (s Solari2000) Name() string {
	return "Solari2000"
}

func (s Solari2000) Load() (map[string]*forecaster.ParkingMeter, error) {
	...
	for _, file := range files {
		...
		go func(file os.DirEntry) {
			defer wg.Done()
			log.Debug().Str("file", file.Name()).Msg("loading Solari2000 file")
...
func (s Solari2000) loadFile(path string) ([]Solari2000Entry, error) {
	...
	entries := make([]Solari2000Entry, 0, len(rows))
	skipped := 0
	for i, row := range rows {
		...
		_ = row[6]
		if duration == "" || !strings.Contains(date, "/") {
			skipped++
			continue
		}

		entry := Solari2000Entry{
			Result:   result,
			Code:     code,
			Zone:     zone,
			Subzone:  subzone,
			Date:     wheretopark.MustParseDateTimeWith(solari2000DateFormat, date),
			Duration: wheretopark.Must(parseSolari2000Duration(duration)),
		}
...
const solari2000DateFormat = "1/2/06 15:04"

func parseSolari2000Duration(str string) (time.Duration, error) {
	a := strings.Split(str, ":")
	h := a[0]
	m := a[1]
	s := a[2]
