...
class RedisStore(
    ...
    private suspend inline fun <reified T> get(namespace: String, id: ParkingLotID): T? {
        val value = client.get("$namespace:$id")
        return value?.let {
            serialization.decodeFromString<T>(it)
        }
    }


    private suspend inline fun <reified T> updateMany(
        namespace: String,
        values: Map<ParkingLotID, T>,
        getExpireAt: (T) -> Instant
    ) {
    ...
    override suspend fun getMetadata(id: ParkingLotID): ParkingLotMetadata? = get(METADATA_NAMESPACE, id)

    override suspend fun getMetadatas(): Map<ParkingLotID, ParkingLotMetadata> = getAll(METADATA_NAMESPACE)

    override suspend fun getState(id: ParkingLotID): ParkingLotState? = get(STATE_NAMESPACE, id)
    ...
    override suspend fun updateMetadatas(metadata: Map<ParkingLotID, ParkingLotMetadata>) =
        updateMany(METADATA_NAMESPACE, metadata)

    override suspend fun updateStates(states: Map<ParkingLotID, ParkingLotState>) =
        updateMany(STATE_NAMESPACE, states, getExpireAt = {
