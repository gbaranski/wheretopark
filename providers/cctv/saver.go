...
func (s *Saver) SavePredictions(img gocv.Mat, id string, cameraID int, captureTime time.Time, spots []Spot, predictions []float32) error {
...
func saveResults(spots []Spot, predictions []float32, path string) error {
	directory := filepath.Dir(path)
	if err := os.MkdirAll(directory, os.ModePerm); err != nil {
		return fmt.Errorf("failed to create directory %s: %w", directory, err)
	}
...
func saveVisualizations(img gocv.Mat, spots []Spot, predictions []float32, path string) error {
	directory := filepath.Dir(path)
	if err := os.MkdirAll(directory, os.ModePerm); err != nil {
		return fmt.Errorf("failed to create directory %s: %w", directory, err)
	}
